"""Shared JAX/gymnax T-Maze base for the Passive and Active variants.

Ported from twni2016/Memory-RL's ``envs/tmaze.py`` (``TMazeBase`` /
``TMazeClassicPassive`` / ``TMazeClassicActive``), reproducing its reward and
(ambiguous-position) observation logic exactly:

    r_t = (1[x_{t+1} >= t - oracle_length] - 1) / (T - 1),  t <= T - 1
    r_T = 1[o_{T+1} = G]

which for the Passive case (``oracle_length=0``) is precisely the reward
definition of the Passive T-Maze M_passive (O=S, L=T-1): the per-step penalty
defaults to ``-1/(T-1)`` and the terminal reward is the goal-hit indicator.
